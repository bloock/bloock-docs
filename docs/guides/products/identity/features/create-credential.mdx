...
The most important thing is that this credential is already a [Verifiable Credential (VC)](../concepts/protocol.mdx#the-verifiable-credential-vc) because it already includes a signature proof, and therefore, this credential would already be valid to be verified.

But BLOOCK Identity's product offers a second proof, related to integrity in blockchain.
The [Sparse Merkle Tree proof](../concepts/protocol.mdx#proofs) is a proof that will be available depending on the [interval time](../concepts/protocol.mdx#issuer-intervals) you have chosen and is related to the Issuer's state.
